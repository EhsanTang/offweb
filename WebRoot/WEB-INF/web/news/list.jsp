 ...
 <input id="url" type="hidden" value="newsList">
	<div class="container mb30" ng-app="app" class="p10" ng-controller="webController">
		<div class="row shadow bg-white mt10 p15 pt0 min-h400">
				<div class="news-list col-sm-12 m0" ng-repeat="item in list.Rows track by $index">
					<div class="col-xs-12 col-sm-8"><a ng-href="go?p=news.detail&id={{item.id}}">{{item.title}}</a></div>
					<div class="col-xs-12 col-sm-4 tr news-date f12 fm">
						<s:property value="@Static@getStatic('L_CLICK').lang"/>:{{item.click}}&nbsp;&nbsp;&nbsp;&nbsp;<s:property value="@Static@getStatic('L_TIME').lang"/>:{{item.date}}
					</div>	
					<div class="cb"></div>	
				</div>
