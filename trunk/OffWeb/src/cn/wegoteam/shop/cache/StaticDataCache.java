...
public class StaticDataCache {
	...
	public static Staticdata getStaticdata(String Code) {
		...
		if (sd == null) {
			...
			if (!MyString.isEmpty(Code))
				sd = staticdataService.findUniqueByHql(
						"from Staticdata where code=:code",
						DataUtils.getMap("code", Code));
		}
		if (sd == null)
			System.out.println("Staticdata:Cade=" + Code + "为空");
	...
	public static Staticdata getPStaticdata(String Code) {
		try {
			...
			System.out.println("根据指定的code获取父staticdata失败:StaticdataCache"
					+ Code);
	...
	public static void removeStaticdata(Staticdata staticdata) {
		...
		// 移除fcodeTypes中的code
		ArrayList<String> al = fcodeTypes.get(staticdata.getPcode() + "_"
				+ staticdata.getType());
	...
	public static void setStaticdata(Staticdata staticdata) {
		...
		if (fcodeTypes.get(staticdata.getPcode() + "_" + staticdata.getType()) == null) {
			...
			fcodeTypes.put(staticdata.getPcode() + "_" + staticdata.getType(),
					al);
		} else {
			ArrayList<String> al = fcodeTypes.get(staticdata.getPcode() + "_"
					+ staticdata.getType());
